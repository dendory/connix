...
def input(cfg = {}, x = {}):
	...
	try:
		...
		try:
			...
			cfg['db'].execute("CREATE TABLE " + table + " (name TEXT, size INT)", []) # Create table using crafted statement
			cfg['db'].commit()
		rowcount = 0
		for name in os.listdir(x['folder']):
			rowcount += 1
			stmt = "INSERT INTO " + table + " (name, size) VALUES (?, ?)"
			if 'mode' in x:
				if x['mode'] == 'merge':
					stmt = "INSERT OR REPLACE INTO " + table + " (name, size) VALUES (?, ?)"
			cfg['db'].execute(stmt, [name, os.path.getsize(os.path.join(x['folder'], name))])
		cfg['db'].commit()
